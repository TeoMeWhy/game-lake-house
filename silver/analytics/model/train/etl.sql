with tb_join as (
  ...
  select t1.match_id,
         ...
         t1.radiant_win,
         t1.win,
         date(t1.dt_match) as dtMatchReference,
         t2.*

  from bronze_gamelakehouse.dota_match_players as t1

  left join silver_gamelakehouse.fs_dota_player_matches as t2
  on date(t1.dt_match) = t2.dtReference and t1.account_id = t2.account_id

  where date(t1.dt_match) >= '{dt_start}'
  and date(t1.dt_match) <= '{dt_stop}'
...
tb_summary(
  ...
  select match_id,
         max(case when radiant_win = True then 1 else 0 end) as radiant_win,
